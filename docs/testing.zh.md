...
- **符号链接权限。** Windows 主机需要启用开发者模式或使用提升权限的 shell 才能创建真实符号链接；否则 `symlink()` 会以 `EPERM` 失败。测试套件以两层方式应对，而不是排除 Windows。只需要目录别名的测试使用无需权限的 `junction` 类型；断言符号链接语义的测试——悬空目标、自引用循环、`readlink` 文本、`lstat` 身份或链接目标身份——保留真实符号链接，并通过 `@deepseek-ai/dsh-platform-probe` 的 `symlinksUsable()` 门控，该函数在临时目录中创建并删除一个真实链接，并按进程缓存结果。门控依据是该能力而非平台：具备该权限的 Windows 主机会运行全部这些测试，而在缺少该权限的 Windows 主机上跳过属于能力限制，而非对 Windows 的排除。
