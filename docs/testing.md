...
## Local platform prerequisites and the Windows baseline

A local run may legitimately report failures that CI does not. Classify a failure before treating it as a regression; an isolated run of the single file is the discriminator.

- **Symlink privilege.** Specs that create a real symbolic link need a Windows host with Developer Mode enabled or an elevated shell; without it `symlink()` fails `EPERM`. They cluster in the filesystem, filesystem-sandbox, workspace, skill, spill, LSP, settings, and session-persistence suites, the workspace-file API, the agent-instruction discovery specs, and the resolver and watch specs under `scripts/`. A directory link whose test does not assert symlink-specific semantics uses the `junction` type instead, which needs no privilege. This is a capability distinction, never a platform one: a Windows host holding the privilege runs every one of them.
- **Timeout budget.** Specs driving real subprocesses, or cryptographic and compression work, take seconds on an idle host and reach the five-second default under a saturated full-suite run — the `scripts/` CLI contracts and the credentials and session-persistence publication specs are the ones observed near the ceiling. They pass in isolation and well inside the budget. A timeout there is contention, not a race.
- **Package-manager resolution.** Repository tooling resolves the package manager from `npm_execpath` and `process.execPath`, never from the caller's `PATH`, so specs are unaffected by a shell that does not expose `pnpm`. Starting a `pnpm run …` script still requires a package-manager executable.
- **Platform-specific suites.** A spec whose contract is another platform's behavior is gated by a capability probe through `describe.skipIf` or `it.skipIf`, not by a bare platform test, so the condition that gives it meaning is the one that skips it.
- **Full-run-only failures.** A spec that fails only inside the whole suite and passes alone, with a message naming the five-second timeout or a saturated resource, is a local baseline limitation. One that reproduces in isolation is a candidate regression until proven otherwise.
