...
const USER_MARKDOWN_LITERAL = 'User-authored literal: # do not render `code` [link](https://example.com)'
...
const FIXTURE_SYSTEM_PROMPT = 'You are the DeepSeek Harness fixture assistant. Answer concisely and call tools when needed.'
...
function buildAlphaLog(): SessionEvent[] {
  ...
  for (let turn = 0; turn < 60; turn++) {
    ...
    const userSeq = push({
      ...
      data: userMessage(text(turn === 59 ? USER_MARKDOWN_LITERAL : `Question ${turn}: fixture history for pagination and rendering checks.`)),
    })
    if (turn === 0) {
      push({
        type: 'session/title',
    ...
    if (withReasoning) blocks.push({ type: 'reasoning', text: `Reasoning ${turn}: collapsible fixture reasoning content.` })
    blocks.push({ type: 'text', text: turn === 59 ? MARKDOWN_FIXTURE : `Answer ${turn}: fixture-generated history response.` })
    if (withTool) {
      ...
      push({ type: 'assistant/message', surfaceOp: 'append', data: { turn, step: 1, message: assistantMessage(text(`Tool result processed (turn ${turn}).`)) } })
  ...
  push({ type: 'user/message', surfaceOp: 'append', data: userMessage(text('Question 72: list all one hundred items in full.')) })
  push({ type: 'step/start', data: { turn: 72, step: 0 } })
  push({
    type: 'assistant/message',
    surfaceOp: 'append',
    data: { turn: 72, step: 0, message: assistantMessage(text('Item 1: first. Item 2: second. Item 3: this item was interrupted halfway')) },
  })
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixtureQuestions: readonly FixtureQuestionItem[] = [
    {
      id: 'harness-profile',
      header: 'Preference',
      question: 'Which kind of Agent/Harness candidate do you want to hire?',
      options: [
        { label: 'Implementation focused (Recommended)', description: 'Prioritizes runtime, tool executor, sandbox, tracing, and production debugging skills.' },
        { label: 'Research potential', description: 'Prioritizes agent understanding, training and evaluation ideas, and long-term growth.' },
        { label: 'Balanced', description: 'Requires engineering ability and agent judgment, with a potentially higher screening bar.' },
      ],
    },
    {
      id: 'work-mode',
      header: 'Approach',
      question: 'Which working style should candidates demonstrate first?',
      options: [
        { label: 'Prototype first (Recommended)', description: 'Validate key assumptions quickly with a working result.' },
        { label: 'Design first', description: 'Settle interfaces, protocols, and risks before implementation.' },
      ],
    },
    {
      id: 'signals',
    ...
    list(id: SessionId): RpcResult<readonly CommandDescriptor[]> {
      ...
      return {
        ...
        value: [
          ...
          { name: 'echo', description: 'fixture: echo the argument', input: { hint: 'text to echo' } },
  ...
  const workspaceFileLines = (path: string): string[] => {
    const name = path.slice(path.lastIndexOf('/') + 1)
    const head = [`# ${name}`, '', 'Sample fixture text for verifying the sidebar preview.', 'Real builds read the matching file from the workspace.']
    return path.includes('demo') || path.includes('huge')
      ? [...head, ...Array.from({ length: 12_000 }, (_, index) => `Line ${index + 5}: long text used to verify pagination and scrolling.`)]
      : head
  }
  ...
  const timingHooks = {
    ...
    beginModelRetry(id: string): void {
      ...
      pushAssistant(sessionId, { type: 'text-delta', index: 0, text: 'Partial response that should be withdrawn' })
    },
    ...
    scheduleModelRetry(id: string, retry = 1, delayMs = 450): void {
      ...
      if (!scenario.stepStarted) {
        ...
        pushAssistant(sessionId, { type: 'block-start', index: 0, blockType: 'text' })
        pushAssistant(sessionId, { type: 'text-delta', index: 0, text: `Retry ${String(retry)} response that should be withdrawn` })
    ...
    completeModelRetry(id: string): void {
      ...
      if (scenario === undefined) throw new Error(`fixture: no model retry scenario for ${id}`)
      retryScenarios.delete(sessionId)
      const completed = 'Complete response after retry'
      beginAssistant(sessionId, scenario.turn, 1)
      pushAssistant(sessionId, { type: 'block-start', index: 0, blockType: 'text' })
      ...
      startReply(
        ...
        userText === 'render markdown'
          ...
          : userText === 'report model'
            ...
            : `Echo: ${userText}. This fixture response streams to verify incremental rendering and finalization.`,
      )
      return sessionOk({ accepted: true as const })
    },
    attachment: (request) => {
  ...
  const approvalInvocation = (): FixtureRemoteEventInvocationFrame => ({
    ...
    request: {
      ...
      reason: 'Persistent fixture approval (it disappears after approval or rejection)',
    },
  })
    ...
    call(channel, endpoint, payload, signal) {
      ...
      switch (endpoint) {
        ...
        case 'skills/list': {
          ...
          return sessionOk({
            skills: [
              { name: 'fixture-demo', description: 'fixture skill sample', whenToUse: 'Only for UI catalog verification', modelInvocable: true },
