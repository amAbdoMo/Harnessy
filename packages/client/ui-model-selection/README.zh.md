...
Web GUI 允许用户通过 `/model` 弹窗或 composer 控件切换既有会话的模型与推理强度。可搜索的双栏对话框会同时应用模型与推理强度，双击则只更改其中一项。两个界面显示同一组按提供方分组的选择；所选模型决定可用推理强度及默认值。完整选择从下一次请求开始生效，运行中的步骤保留启动时的路由。没有适配器可以服务该会话时，composer 保持停用。
