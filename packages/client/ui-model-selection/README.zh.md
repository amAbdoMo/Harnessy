...
Web GUI 允许用户通过 `/model` 弹窗或 composer 模型控件切换既有会话使用的模型与推理强度。composer 会打开一个可搜索的双栏对话框：先单击模型、再单击推理强度即可同时应用；双击任一项则只更改该项。两个界面呈现同一组按提供方分组的选择；所选模型决定可用的推理强度名称与默认值。完整选择从下一次请求开始生效；运行中的步骤保留其启动时的模型与推理强度。如果没有适配器可以服务会话路由，composer 会保持停用，直至路由恢复可用。
...
模型按提供方分组。composer 对话框显示展示名称和持久模型 ID，在用户输入时本地筛选，并在暂存模型后更新推理强度栏。普通单击模型后再普通单击强度会应用组合并关闭对话框。双击模型时，新模型若支持当前强度则保留，否则采用新模型默认值；双击强度则保留当前模型。`/model` 弹窗仍应用所选模型的默认推理强度。适配器没有推理元数据时会立即完成模型选择；不存在任意推理强度输入。
