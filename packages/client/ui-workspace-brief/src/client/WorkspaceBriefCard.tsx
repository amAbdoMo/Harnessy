import { IconBrowseOutlineRegular, MarkdownText, StateDot } from '@deepseek-ai/dsh-client-ui-primitives'
    ...
    <article className={css.card} data-state={state} aria-label={t('card.title')}>
      <header className={css.header}>
        <span className={css.mark} aria-hidden="true">
          {state === 'error' ? <StateDot state="error" /> : <IconBrowseOutlineRegular size={14} />}
        </span>
