...
function mount(options: MountOptions = {}) {
  ...
  const useSessions = <Selected,>(selector: (state: {
    byId: Record<string, { cwd?: string; retainedBy: { mainView: number } }>
  }) => Selected): Selected => selector({
    byId: { 'session-1': { cwd: WORKSPACE, retainedBy: { mainView: 1 } } },
  })
...
/** Wait for one asynchronously loaded option without mutating the menu in a polling callback. */
async function optionsContaining(control: HTMLElement, label: string): Promise<string[]> {
  fireEvent.click(control)
...
/** Persist the expanded card's staged edits. */
function save(name: string): void {
  fireEvent.click(within(card(name)).getByRole('button', { name: en.cardSaveChanges }))
}
...
describe('Subagents picker catalog source', () => {
  it('sources a runtime-resolvable role from the global catalog', async () => {
    ...
    expect(await optionsContaining(modelControl('Code'), 'DeepSeek Chat')).toContain('DeepSeek Chat')
  ...
  it('sources a role whose backend owns its model space from that backend\'s catalog', async () => {
    ...
    expect(await optionsContaining(modelControl('Code'), 'deepseek/deepseek-v4.1-flash')).toContain('deepseek/deepseek-v4.1-flash')
  ...
  it('keeps a saved route the new source cannot advertise, and lets it go', async () => {
    ...
    expect(await optionsContaining(model, `gone ${en.modelUnavailableSuffix}`)).toContain(`gone ${en.modelUnavailableSuffix}`)
...
describe('Subagents picker reasoning levels', () => {
  ...
  it('offers a backend-owned role the levels that backend accepts', async () => {
    ...
    expect(await optionsContaining(modelControl('Code'), 'local-model')).toContain('local-model')
  ...
  it('shows a stored level again after a reload, as one of the levels on offer', async () => {
    ...
    expect(within(card('Code')).getAllByText('local-model').length).toBeGreaterThan(0)
    expect(within(card('Code')).getAllByText(en.effortMedium).length).toBeGreaterThan(0)
  })
