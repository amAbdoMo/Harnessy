...
/** Required services: slots, locale, configuration forms, and the roster and session Remote namespaces. */
export const inject = [
  'slots', 'locale', 'remote', 'remote.subagentRoster', 'remote.session', 'configForms',
]
 ...
 * @param ctx - Client root context carrying slots, locale, and configuration forms.
 */
export function apply(ctx: ClientContext): void {
  ...
  const scope = ctx.configForms.get<SubagentSettings>(SUBAGENT_ROSTER_NAMESPACE)
  ...
  const operations: SubagentsOperations = {
    ...
    write: async (patch, expectedRevision) => {
      ...
      if (!(await scope.mutate(ops, expectedRevision))) {
        throw new Error('Subagent settings were not saved')
      }
    },
