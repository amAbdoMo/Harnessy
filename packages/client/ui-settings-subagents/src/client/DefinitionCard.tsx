...
/** Catalog presentation resolved for the backend and route in an editable draft. */
export interface SubagentDefinitionPicker {
...
export interface DefinitionCardProps {
  ...
  /** Resolve the model catalog for the backend and route currently staged in the editor. */
  readonly pickerFor: (
    backend: string,
    route: SubagentDefinition['model']['route'],
  ) => SubagentDefinitionPicker
  /** Resolve whether the runtime currently staged by this role can accept work. */
  readonly setupRuntimeFor: (backend: string) => SubagentSetupRuntimeState
...
export function DefinitionCard(props: DefinitionCardProps): ReactNode {
  ...
  const shownPicker = pickerFor(shown.execution.backend, shown.model.route)
  const draftPicker = pickerFor(draft.execution.backend, draft.model.route)
  ...
  const effort = subagentEffortName(shownPicker.choices, route)
  const summaryModelName = modelNameOf(shown.model, shownPicker.choices, t)
  const configurationModelName = modelNameOf(draft.model, draftPicker.choices, t)
  ...
  const setup = subagentSetupResult(draft, draftPicker.source, setupRuntimeFor(draft.execution.backend))
            ...
            <section className={css.editorSection} aria-labelledby={`${fieldId}-run-title`}>
              ...
              <div className={css.runGrid}>
                <ModelEffortPicker
                  ...
                  model={draft.model}
                  choices={draftPicker.choices}
                  directory={draftPicker.directory}
                  source={draftPicker.source}
