...
- **The Host resolves, the browser shows source.** The resolved roster read supplies the values a delegation runs with and the per-field override source; the page reads it afresh after each accepted write, so inherited/overridden state converges on the document that was committed.
