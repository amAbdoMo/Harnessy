...
import { capabilityKey, capabilitySource, routeCapabilities } from '../src/client/capability.ts'
import type { CapabilitySourceState } from '../src/client/capability.ts'
import { CapabilitySource } from '../src/client/CapabilitySource.tsx'
...
/**
 * Render one row's source.
 * @param state - the reduced state to render.
 * @returns the container's text content.
 */
function text(state: Parameters<typeof CapabilitySource>[0]['state']): string {
  const { container } = render(<CapabilitySource state={state} now={NOW} t={t} />)
...
describe('deriving what a row renders', () => {
  ...
  it('leaves an id-only claim as a claim, never as a capability', () => {
    const state = capabilitySource(view({
  ...
  it('reports a disabled layer rather than an absence of claims', () => {
    expect(capabilitySource(view({ enabled: false }), UNDECLARED)).toEqual({ kind: 'disabled' })
  })

  it('says nothing about a model no database claims', () => {
    expect(capabilitySource(view(), UNDECLARED)).toEqual({ kind: 'none' })
  })

  it('says nothing for a row the Host does not report at all', () => {
    expect(capabilitySource(undefined, UNDECLARED)).toEqual({ kind: 'none' })
  })
  ...
  it('reports no public claim beside a declaration when no catalog matched', () => {
    expect(capabilitySource(view(), { declaresNoReasoning: false, efforts: ['low', 'max'] }))
  ...
  it('reports nothing applied for a declared row even when the layer is disabled', () => {
    expect(capabilitySource(view({ enabled: false }), {
...
describe('what a row reads', () => {
  ...
  it('dates a fetch against the render clock when no clock is supplied', () => {
    const state: CapabilitySourceState = {
      kind: 'public',
      levels: ['low'],
      source: 'models.dev',
      provider: 'cortecs',
    ...
    const { container } = render(<CapabilitySource state={state} t={t} />)
...
describe('the lookup the page builds', () => {
  ...
  it('shows the same model differently through two routes', () => {
    ...
    expect(capabilitySource(matched, UNDECLARED)).toMatchObject({ kind: 'public' })
    expect(capabilitySource(suggested, UNDECLARED)).toMatchObject({ kind: 'id-only' })
  })
})
