...
Open the Models page from the Settings navigation to see every product-visible configured provider as a row. The Harnessy profile omits the stock `deepseek-official` row and its onboarding step; custom DeepSeek-compatible providers remain ordinary configurable routes. A whole-section provider whose key is not configured anywhere renders as its open setup card instead, but only in the first-run posture and only until the user closes that card. Each card kind owns its own open state, so closing one never discards a draft in another.
...
The collapsed 自定义设置 fold carries the curated extras: `baseURL` for both families (the deepseek placeholder shows the public endpoint), each adapter's model catalog, and the **display name** and **API protocol** of a pi-ai route the adapter does not ship. Profile `headers` remain deployment configuration in `settings.yaml` or Cordis config and have no Models-page editor. The Provider ID stays fixed: it is the settings key, the name every other namespace and every logged session references, and the stem of a credential reference the page cannot read back to move. Reasoning is deliberately not among the *card's* fields: it is a per-model capability, so a provider-scoped control could only be set to a value some models reject. Each pi-ai model row declares it inside the row's own disclosure instead — the declaration itself (the adapter's own answer, an explicit support with its offered levels, or an explicit refusal), the offered effort levels, and the level that model's requests default to — and the composer's picker then offers that one model exactly the levels it declares. A row that declares nothing says which route to a set applies to it: a route the adapter ships already inherits the catalog's levels, while a route it does not has to import them from a fetch or have them chosen here. Each DeepSeek row edits `id`, optional display `name`, and optional `contextWindow`/`maxTokens`; existing fields outside that curated set survive edits.

### Where a model's effort levels come from

A model row's disclosure also reports the source of the levels that apply to it, read from the `modelCapabilities/inspect` Remote: the Host resolves and matches, and this page renders the answer. A row the user has declared — or explicitly refused — shows **Configured** as its source, and a public claim the catalogs state but the declaration outranks is reported beside it as not applied. A row that declares nothing shows what actually applies: the levels, then **Public metadata** with the database that answered, the matched provider entry, and whether that answer was live, cached, or bundled. A bundled catalog has no fetch time, so its line reads **Bundled with Harnessy** rather than inventing one; a fetched one is dated through the shared relative-time buckets every other dated row uses.

An id-only match is never presented as a capability. A row whose model a public database names without matching the route reads **Public metadata may describe this model, but the route could not be verified**, and disagreeing claims read **Multiple public providers report different effort sets for this model** with each competing claim listed underneath — no winner is chosen and nothing is applied. A model no database states keeps the plain undeclared row, because a public database having no record of a model is not a fault to report. When the layer is off, the row says **Public metadata is disabled** rather than showing an absence of claims that a disabled layer would also produce.

The distinction between an applied capability and a suggestion is carried in words, never in colour alone. The block is a description list, so every term is paired with its value for assistive technology, and every string it prints is locale-owned copy.

### Adding and deleting providers

The add flow is a card carrying the dormant-directory provider select — a bare-mounted `llm-pi-ai` offers its whole installed catalog before any route exists. **Add a custom provider** declares a route pi-ai does not ship; the create card asks for a unique **Provider ID**, an endpoint, a protocol, and at least one uniquely-identified model, because nothing can default those. **Fetch available models** asks the `llm/discoverModels` Remote about the endpoint the form shows, so adding a provider is one pass instead of save-then-return; the reply opens a searchable picker rather than being written, and nothing is written until **Add selected**. Each selected candidate copies its id, display name, context window, and output-token cap into the editable row when disclosed, while an existing row retains its user-tuned values. A candidate that states its reasoning-effort levels — the endpoint's own answer for a route the adapter does not ship, or the installed catalog's for one it does, which is what the picker names above the list — has exactly those levels adopted with it, so the row starts from the provider's answer rather than from a set this page invented. Search matches model ids and optional display names without clearing hidden selections. **Select all** adds the visible results, while **Deselect all** clears the entire selection so hidden results cannot be adopted accidentally. A row is deletable only when the user layer alone carries it (removal restores the composition base), and its confirmation dialog names the provider.
