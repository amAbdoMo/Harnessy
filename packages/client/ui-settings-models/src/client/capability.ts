/**
 * What one configured model row's reasoning source is, derived from the
 * Host's own answer.
 *
...
/**
 * The key one configured model's source is held under.
 *
 ...
 * configured model for the Host to have resolved, so it shows no source
 ...
 * @param capabilities - the page's source by {@link capabilityKey}.
...
/** How one row's source renders, once the row's own declaration is known. */
...
/**
 * Decide how one row renders its source.
 *
...
export function capabilitySource(
  ...
  /* v8 ignore next -- a route whose rows the Host does not report renders no source. */
