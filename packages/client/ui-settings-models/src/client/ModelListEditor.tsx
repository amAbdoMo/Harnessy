...
import {
  declaredCapability, declaredDefaultReasoningEffort, declaredReasoningEfforts,
  formatCapacity, modelReasoningMode, parseCapacity, reasoningEffortsMap,
  type ModelReasoningMode,
} from './DeepSeekModelsEditor.tsx'
import type { ModelsOperations } from './operations.ts'
import { capabilitySource } from './capability.ts'
import { CapabilitySource } from './CapabilitySource.tsx'
import type { DeepSeekModelDraft } from './DeepSeekModelsEditor.tsx'
import type { en } from './locales.ts'
import { ModelRow } from './ModelRow.tsx'
import styles from './ModelsSection.module.css'

/** Copy lookup a row renders with, including the params its source needs. */
...
export function ModelListEditor(props: ModelListEditorProps): ReactNode {
  const { models, onChange, probe, operations, t, disabled, onBusyChange, efforts } = props
            ...
            extra={(
              <>
                {efforts.length > 0 && (
                  <label className={styles['modelField']}>
                    ...
                    <Select
                      ...
                      onChange={(mode) => { setReasoningMode(index, mode) }}
                    />
                  </label>
                )}
                {modelReasoningMode(model) === 'supported' && (
                  <CapabilityFields model={model} index={index} efforts={efforts} t={t} disabled={disabled}
                ...
                {efforts.length > 0 && modelReasoningMode(model) === 'inherit'
                  ...
                  : null}
                <CapabilitySource
                  state={capabilitySource(props.capability?.get(textOf(model, 'id')), {
                    ...modelReasoningMode(model) === 'supported'
                      ? { efforts: declaredReasoningEfforts(model, efforts) }
                      : {},
