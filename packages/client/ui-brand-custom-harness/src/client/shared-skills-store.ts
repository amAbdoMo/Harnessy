...
export interface SharedSkillsRowState {
  status: ConfigFormSnapshot<SharedSkillsSettings>['status']
  enabled: boolean
  directory: string
...
type SharedSkillsRowActions = {
  sync: (state: SharedSkillsRowState, snapshot: ConfigFormSnapshot<SharedSkillsSettings>) => void
}

function baseDirectory(snapshot: ConfigFormSnapshot<SharedSkillsSettings>): string {
...
export function createSharedSkillsRowStore(): EngineStoreHandle<SharedSkillsRowState, SharedSkillsRowActions> {
  return defineStore({
    ...
    actions: {
      sync: (state, snapshot: ConfigFormSnapshot<SharedSkillsSettings>) => {
