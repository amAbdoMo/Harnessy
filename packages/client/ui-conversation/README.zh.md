...
Session header action 的 owner 可以请求一个已注册 View 和可选 focus key。shell 会先激活该 View，再转发 focus request，因此状态表面无需拥有 Chat 状态即可跳转到准确的持久化 Chat event。
