...
interface SearchBlockCommon {
   ...
   * folds the pre-cap `total` into the label so the card never presents a
...
/**
 * The banner summary. When the search was capped it states the shown and total counts so
 * the retained count and the pre-cap total sit in one clause (mirroring the read
 * card's line count); when it was not capped it is a plain count of what the
 * card holds. The match/file unit for grep or path unit for glob trails
