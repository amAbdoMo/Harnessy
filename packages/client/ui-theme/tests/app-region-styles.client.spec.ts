...
/** Harnessy's visible desktop menu row is also a draggable titlebar. */
const HARNESSY_TITLEBAR_SELECTOR = '.desktopTitlebar'
...
describe('window drag-region ownership', () => {
  ...
  it('declares the window drag surface only on the platform and Harnessy titlebars', () => {
    ...
    // Windows caption row and Harnessy's visible desktop menu row. Another
    // declaration, a per-sheet row rule, or a darwin band on the frame fails here.
    expect(dragOwners()).toEqual([
      { file: WINDOWS_CAPTION, selectors: [HARNESSY_TITLEBAR_SELECTOR, WINDOWS_CAPTION_SELECTOR] },
