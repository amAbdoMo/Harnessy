...
const PROTECTED_VALUE = '<saved in protected storage; replace to update>'
...
export class McpManagerController extends TypertRemoteService {
  ...
  private async readSynchronizedVault(): Promise<SynchronizedVault> {
    ...
    const imported = restoreProtectedValues(parseConfigurationText(text), vault)
    const changed = profileKeyOfVault(imported) !== profileKeyOfVault(vault)
    if (changed) await this.writeVault(this.credentials(), imported)
    await this.writeConfiguration(imported)
    return { vault: imported, imported: changed }
  }

  private async writeConfiguration(vault: McpVault): Promise<void> {
    const path = this.requiredConfigurationPath()
    const text = `${JSON.stringify(redactedConfiguration(vault), null, 2)}\n`
...
/** Keep saved credentials when an edited public document retains their marker. */
function restoreProtectedValues(incoming: McpVault, saved: McpVault): McpVault {
  const previous = new Map(saved.servers.map(server => [server.id, server]))
  const servers = incoming.servers.map((server): StoredServer => {
    const existing = previous.get(server.id)
    if (server.transport === 'streamable-http') {
      if (server.authorization !== PROTECTED_VALUE) return server
      if (existing?.transport !== 'streamable-http' || existing.authorization === undefined) {
        throw rejected(server.id, 'replace the authentication marker with a value for this new server')
      }
      return { ...server, authorization: existing.authorization }
    }
    const environment = Object.fromEntries(Object.entries(server.environment).map(([key, value]) => {
      if (value !== PROTECTED_VALUE) return [key, value]
      if (existing?.transport !== 'stdio' || existing.environment[key] === undefined) {
        throw rejected(server.id, `replace the protected environment marker for ${key}`)
      }
      return [key, existing.environment[key]]
    }))
...
/** Write connection details without copying protected values into the editable file. */
function redactedConfiguration(vault: McpVault): McpVault {
  return {
    version: 1,
    servers: vault.servers.map(server => server.transport === 'streamable-http'
      ? { ...server, ...server.authorization === undefined ? {} : { authorization: PROTECTED_VALUE } }
      : {
        ...server,
        environment: Object.fromEntries(Object.keys(server.environment).map(key => [key, PROTECTED_VALUE])),
      }),
  }
}
