...
Create 按以下顺序解析工作目录：已选 Workspace、请求中显式的 `cwd`，然后是实时 `session-workspace` 设置。默认设置使用 Host 启动目录。远程网站模式会在配置的绝对父目录下为每条 Session 创建独立的 `Harnessy Remote Work - <id>` 子目录，再创建 Agent。现有 Session、fork 与 subagent 保留已记录或继承的目录。远程网站模式隔离本地临时文件以便清理；它不是工具访问策略，也不会把 Session 变成仅限 MCP。
