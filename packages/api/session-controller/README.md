...
Create resolves its working directory in this order: selected Workspace, explicit request `cwd`, then the live `session-workspace` setting. The default setting uses the Host launch directory. Remote website mode creates a distinct `Harnessy Remote Work - <id>` child under the configured absolute parent directory before Agent creation. Existing Sessions, forks, and subagents retain their recorded or inherited directory. Remote website mode isolates local scratch files for cleanup; it is not a tool-access policy and does not make the Session MCP-only.
