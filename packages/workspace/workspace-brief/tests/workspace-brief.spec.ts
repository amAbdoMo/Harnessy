...
function bench(options: {
  ...
  const shell = {
    ...
    execute: vi.fn(async (request: { signal?: AbortSignal }) => {
      calls.shell += 1
      const result = typeof options.shell === 'function'
        ? await options.shell(request.signal ?? AbortSignal.abort())
        : options.shell ?? shellResult()
...
describe('WorkspaceBriefRunner', () => {
  ...
  it('rejects every unsupported suffix before touching the workspace', async () => {
    ...
    expect(test.shell.execute).not.toHaveBeenCalled()
  })
