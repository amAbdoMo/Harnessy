...
它必须挂载在 `@deepseek-ai/dsh-llm` 之后，因为该服务是必需的。设置接缝是可选的：挂载 `@deepseek-ai/dsh-settings` 时，可以通过当前 profile 的配置表单修改实时字段；不挂载时，组合行就是全部配置。
...
插件拥有 `model-capabilities` 设置命名空间，因此同样的取值也可以在当前 profile 中以 `model-capabilities.publicMetadata` 配置：
...
写入是通过设置接缝向当前 profile 补丁执行的一次 `llm-pi-ai` 合并。接缝会整体替换数组，因此被触及的路由会贡献其完整模型列表，只有匹配的行发生变化——其余每一行的每个字段都按原样重述，这正是让写入不会丢掉它本无意触及的配置的机制。当没有任何可写入项时，接缝根本不会被调用，因此对一个已完全声明的部署执行 `write` 会让 profile 逐字节不变。
...
- [dsh-settings](../../settings/settings/README.zh.md) —— `model-capabilities.publicMetadata` 与同步所写入的 `llm-pi-ai` 配置背后的 profile 表单。
...
- **推理强度是唯一的能力** —— 视觉、工具、模态、结构化输出与上下文上限都还未读取，尽管 ModelCapabilitySource.capability 与解析结果词汇的形态足以承载它们。Models 页面报告的就是这一种能力的来源信息，后续能力会扩展同一份投影。
...
- **同步只写入设置接缝所拥有的内容** —— 它编辑当前 profile 的 `llm-pi-ai` 覆盖，因此路由来自不可编辑的启动器或 home 覆盖时没有可供同步持久化的去处；`check` 仍会报告它。
...
- 同步所写入的路由来自已解析的 `llm-pi-ai` 段落，而不是仅来自 profile 覆盖：接缝公开的是有效值，而该值是覆盖的超集，因此重述被触及路由的模型列表不会丢掉用户自己的字段。
