...
Mount it after `@deepseek-ai/dsh-llm`; that service is required. The settings seam is optional: with `@deepseek-ai/dsh-settings` mounted, live fields can be changed through the active profile's configuration form, and without it the composition row is the whole configuration.
...
The plugin owns the `model-capabilities` settings namespace, so the same values are configurable from the active profile as `model-capabilities.publicMetadata`:
...
A composition with its own catalog source skips the plugin and registers directly. `origin` states where each catalog came from — `fixture`, `bundled`, `live`, or `cache` — and travels into the source of every answer.
...
`resolvePublicCapability(catalogs, modelId, request)` returns the same answer the source answers with, plus the source and the suggestions the source deliberately withholds. Diagnostics use it to explain a chain: a `resolved` result names the matched provider entry and how it matched, and an `unresolved` one carries the model-id-only claims it found instead.
...
| File | Role |
|---|---|
| [`src/types.ts`](src/types.ts) | The catalog, match, source, and resolution vocabulary |
...
The write is one `llm-pi-ai` merge through the settings seam into the active profile patch. The seam replaces arrays wholesale, so a touched route contributes its complete models list with only the matching rows changed — every other field of every row is restated as it stands, which is what keeps the write from dropping configuration it did not mean to touch. When nothing is writable the seam is not called at all, so a `write` against a fully declared deployment leaves the profile byte-identical.
...
- [dsh-settings](../../settings/settings/README.md) — the profile-backed forms behind `model-capabilities.publicMetadata` and the `llm-pi-ai` configuration a sync writes.
- [dsh-cmdline](../../boot/cmdline/README.md) — how an app plugin owns its own flag family and its `--help`.
- [Models page](../../client/ui-settings-models/README.md) — where this layer's source is displayed.
...
- **Reasoning efforts are the only capability** — vision, tools, modalities, structured output, and context limits are not read, though `ModelCapabilitySource.capability` and the resolution vocabulary are shaped to carry them. The Models page reports this one capability's source, and a later capability extends the same projection.
...
- **A sync writes only what the settings seam owns** — it edits the active profile's `llm-pi-ai` override, so a deployment whose routes come from a non-editable launcher or home overlay has nowhere for a sync to persist; `check` still reports it.
...
- The route a sync writes is the resolved `llm-pi-ai` section, not the profile override alone: the seam exposes the effective value, and that value is a superset of the override, so restating a touched route's models list cannot drop the user's own fields.
