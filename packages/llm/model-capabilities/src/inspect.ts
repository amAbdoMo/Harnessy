...
/** One configured model row, as the active profile states it. */
 ...
 * the composition entry and one supplied by the profile override are both read.
...
/**
 * Read every configured model's capability source for a Client surface.
 * @param routes - the configured routes.
 * @param store - the mounted catalog store.
 * @param enabled - whether the public metadata layer answers under the policy.
...
export function explainModelCapability(
  ...
  const resolved: ModelCapabilityResolved | undefined = resolution.kind === 'resolved'
    ? {
      ...
      provider: resolution.source.provider as string,
      match: resolution.source.match,
      origin: resolution.source.origin as SelectedPublicCatalog['state'],
      ...resolution.source.fetchedAt === undefined ? {} : { fetchedAt: resolution.source.fetchedAt },
    }
    : undefined
...
export interface ModelCapabilityInspection {
  ...
  readonly model: string
  /**
   * The configured route's own fields, as the active profile states them.
   *
   * A write restates this row into the route's `models` list, because the
   * settings seam merges a patch over the profile override and replaces arrays
 ...
 * @param modelProfile - the configured row's fields, as the active profile states them.
