...
/** Read one live profile section from the schema-derived settings service. */
function settingsSection(ctx: Context, namespace: string): unknown {
  return ctx.get('settings')?.describe().find(section => section.ns === namespace)?.value
}
...
async function runOnce(
  ...
  const policy = readCliPolicy(settingsSection(ctx, MODEL_CAPABILITIES_NAMESPACE))
  if (policy === undefined || !policy.enabled) {
    ...
    write('enable it in the active profile configuration or composition entry to use these commands')
    return EXIT_OK
  }
  ...
  const routes = readConfiguredRoutes(settingsSection(ctx, LLM_PI_AI_NAMESPACE))
  if (target !== undefined) {
  ...
  if (sync === 'write') {
    ...
    try {
      ...
      err(`model-capabilities: writing the profile configuration failed: ${String(error)}\n`)
      return EXIT_FAILURE
    }
  }
