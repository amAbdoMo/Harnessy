 ...
 *
 * The section resolves under the composition entry as its base and the active
 * profile's live configuration over it, so a deployment states a default and
 * a user overrides it without a separate settings document.
 *
...
/** The whole live configuration section. */
...
/** Runtime configuration with a live public-metadata policy reference. */
export interface Config {
  /** Current public metadata policy. */
  readonly publicMetadata: Volatile<PublicMetadataConfig>
}
...
/** The section schema. */
export const ModelCapabilitiesConfigSchema: z<Config> = z.object({
  ...
  }).default(MODEL_CAPABILITIES_DEFAULTS.publicMetadata).volatile(),
})
