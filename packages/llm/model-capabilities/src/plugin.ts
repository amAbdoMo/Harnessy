...
import type { Config as ModelCapabilitiesRuntimeConfig, ModelCapabilitiesConfig } from './config.ts'
...
export function apply(
  ctx: Context,
  config: ModelCapabilitiesRuntimeConfig | ModelCapabilitiesConfig = MODEL_CAPABILITIES_DEFAULTS,
  openStore: (options: PublicCatalogStoreOptions) => PublicCatalogStore = createPublicCatalogStore,
): void {
  const currentConfig = (): ModelCapabilitiesConfig => ({
    publicMetadata: 'get' in config.publicMetadata
      ? config.publicMetadata.get()
      : config.publicMetadata,
  })
  assertServiceableConfig(currentConfig())
  const store = openStore({
    policy: () => resolvePolicy(currentConfig()),
  ...
  const startRefresh = (): void => {
    const policy = resolvePolicy(currentConfig())
  ...
  // The Models page's source read, over the same store and the same
  ...
  ctx.on('settings/document-updated', (namespace) => {
    if (namespace === MODEL_CAPABILITIES_NAMESPACE) startRefresh()
  })
