 ...
 * exists so a browser surface can render source without importing a Host
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    /** The Models page's capability-source read. */
...
export class ModelCapabilitiesInspector extends TypertRemoteService {
  ...
  async remoteExportInspect(signal: AbortSignal): Promise<ModelCapabilityInspectionView[]> {
    ...
    const config = settings?.describe()
      .find(section => section.ns === MODEL_CAPABILITIES_NAMESPACE)?.value as ModelCapabilitiesConfig | undefined
    const enabled = config === undefined ? true : resolvePolicy(config).enabled
    const routes = readConfiguredRoutes(settings?.describe().find(section => section.ns === LLM_PI_AI_NAMESPACE)?.value)
