...
/** Write the deployment's legacy settings document for one-time import. */
...
const policyPath = join(process.cwd(), 'model-capabilities-policy.patch.yml')
const policyEnabled = !process.argv.includes('--models-disabled')
const policyRefresh = process.argv.includes('--models-never-refresh') ? 'never' : 'manual'
await writeFile(policyPath, `- id: model-capabilities
  config:
    publicMetadata:
      enabled: ${String(policyEnabled)}
...
const ctx = await bootProductionProfile({
  ...
  profile: 'headless',
  overlayPaths: [resolveConfigPath(configPath, undefined), policyPath],
  prepare: (hostCtx) => {
...
try {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    runs,
    exports,
    settings: await readFile(ctx.settings.documentPath, 'utf8'),
  })}\n`)
} finally {
