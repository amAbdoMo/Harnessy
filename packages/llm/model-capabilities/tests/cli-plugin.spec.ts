...
describe('writing the profile configuration', () => {
...
async function mount(
  ...
  if (options.settings !== false) {
    ...
    ctx.provide('settings', {
      describe: () => [
        { ns: 'model-capabilities', value: { publicMetadata: { enabled: true, refresh: 'manual', cacheTtl: '7d' } } },
        ...(stored === undefined ? [] : [{ ns: 'llm-pi-ai', value: stored }]),
      ],
...
describe('the command-line surface', () => {
  ...
  it('reports a refused settings write and exits failure', async () => {
    ...
    await mounted.settle()
    expect(mounted.exits).toEqual([1])
    expect(mounted.err()).toContain('writing the profile configuration failed')
    await mounted.ctx.fiber.dispose()
  })
  ...
  it('refuses a write when this composition mounts no settings service', async () => {
    ...
    ctx.provide('settings', {
      describe: () => [{
        ns: 'model-capabilities',
        value: { publicMetadata: { enabled: true, refresh: 'manual', cacheTtl: '7d' } },
      }],
    })
  ...
  it('fetches nothing when only a refresh was asked for and no policy governs it', async () => {
    ...
    ctx.provide('settings', {
      describe: () => [
        { ns: 'model-capabilities', value: { publicMetadata: { enabled: true, refresh: 'never', cacheTtl: '7d' } } },
        { ns: 'llm-pi-ai', value: ROUTES },
      ],
    })
    ctx.provide(MODEL_CAPABILITY_STORE_SERVICE, store())
    let stderr = ''
