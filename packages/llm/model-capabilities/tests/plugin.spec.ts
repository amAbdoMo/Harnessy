 ...
 * @param options - profile configuration, network behaviour, and adapter hooks.
...
describe('enabled: false', () => {
  it('registers nothing that declares a capability and makes no request', async () => {
    const harness = await mount({
      config: { publicMetadata: { ...MODEL_CAPABILITIES_DEFAULTS.publicMetadata, enabled: false } },
    })
    try {
      const models = await harness.ctx.llm.discoverModels('llm-example', {
        provider: 'acme',
...
describe('refresh modes', () => {
  it.each(['manual', 'never'] as const)('makes no automatic request under refresh: %s', async (refresh) => {
    const harness = await mount({
      config: { publicMetadata: { ...MODEL_CAPABILITIES_DEFAULTS.publicMetadata, refresh } },
    })
    try {
      const models = await harness.ctx.llm.discoverModels('llm-example', {
        provider: 'acme',
