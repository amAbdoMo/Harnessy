...
describe('the bundled snapshot age diagnostic', () => {
  it('says nothing while the snapshot is inside the advisory window', () => {
    const sink = logger()
    reportSnapshotAge(sink, new Date(NOW - 89 * DAY_MS).toISOString(), NOW)
    reportSnapshotAge(sink, new Date(NOW).toISOString(), NOW)
    expect(sink.warnings).toEqual([])
  })

  it('warns, without failing, once the snapshot reaches the window', () => {
    const sink = logger()
    reportSnapshotAge(sink, new Date(NOW - 90 * DAY_MS).toISOString(), NOW)
    reportSnapshotAge(sink, new Date(NOW - 400 * DAY_MS).toISOString(), NOW)
    expect(sink.warnings).toEqual([
  ...
  it('names an unreadable generation time rather than reporting a nonsense age', () => {
    const sink = logger()
    reportSnapshotAge(sink, 'not-a-time', NOW)
  ...
  it('is wired to the bundled artifact the plugin actually ships', async () => {
    ...
    const sink = logger()
    reportSnapshotAge(sink, bundledSnapshotMetadata.generatedAt, NOW)
