...
export const Config = z.object({
  ...
  lanes: CommandCodeDelegationFields.lanes.volatile(),
  projects: CommandCodeDelegationFields.projects.volatile(),
})
...
export class CommandCodeController extends TypertRemoteService {
  static inject = ['typert', 'tools', 'subprocess']

  static Config = Config

  private readonly readDelegationSettings: () => CommandCodeDelegationSettings
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.readDelegationSettings = () => ({
      maxConcurrentRuns: config.maxConcurrentRuns.get(),
      timeoutMs: config.timeoutMs.get(),
      maxTurns: config.maxTurns.get(),
      lanes: [...config.lanes.get()],
      projects: { ...config.projects.get() },
    })
    this.limiter = new CommandCodeRunLimiter(() => this.readDelegationSettings().maxConcurrentRuns)
  ...
  delegation(workspace: string | null, signal: AbortSignal): CommandCodeDelegationView {
    ...
    return commandCodeDelegationView(this.readDelegationSettings(), workspace)
  }

  /** The tools' view of this controller. */
  private delegationApi(): CommandCodeDelegationApi {
    return {
      viewFor: workspace => commandCodeDelegationView(this.readDelegationSettings(), workspace),
