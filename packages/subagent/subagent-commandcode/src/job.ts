 ...
 * foreground path does it, and it becomes the job's terminal result.
 ...
 *
 * Coarse activity is published through the registry's progress channel. The
 * terminal outcome carries the same bounded text a foreground delegation
 * returns: the final answer, or the product-owned diagnostic with any preserved
 * partial answer.
 * @param parts - resolved lane policy, brief, workspace, and the plugin surface.
 * @param job - optional generic job handle receiving coarse progress updates.
 * @returns the hooks the generic job runtime drives.
 */
export function startCommandCodeJob(parts: CommandCodeJobParts, job?: JobHandle): CommandCodeJob {
  ...
  const done = (async (): Promise<JobOutcome> => {
    ...
    const record = (outcome: JobOutcome): JobOutcome => {
      ...
      return { ...outcome, ...terminal === undefined ? {} : { result: terminal } }
    }
    ...
    try {
      return record(await execute(
        parts,
        controller.signal,
        (next) => { job?.updateProgress(`${parts.label}: ${next}`) },
        (text) => { terminal = text },
      ))
    } catch (error: unknown) {
