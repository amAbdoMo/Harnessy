...
describe('background delegation against the real job registry', () => {
  it('yields only new coarse activity while the run is live', async () => {
    ...
    expect(jobs.read(id).job.progress).toBeUndefined()
    await waitFor(() => live.started())
    live.emit('reading')
    expect(jobs.read(id).job.progress).toBe('Command Code · Review: reading')
    // The same category is not repeated.
    expect(jobs.read(id).chunks).toEqual([])
    live.emit('editing')
    expect(jobs.read(id).job.progress).toBe('Command Code · Review: editing')
  ...
  it('makes the completed final answer collectible after settlement', async () => {
    ...
    expect(jobs.read(id).job.progress).toBe('Command Code · Review: finalizing')

    live.settle({ output: [{ type: 'text', text: 'the final answer' }], stopReason: 'completed' })
    await jobs.wait(id, 1_000)

    const completed = jobs.read(id)
    expect(completed.job.status).toBe('completed')
    expect(completed.result).toBe('the final answer')
  ...
  it('exposes the product-owned diagnostic and the preserved partial answer for a failed run', async () => {
    ...
    expect(read.result).toContain('Product delegation failure')
    expect(read.result).toContain('the run reached its turn limit before a final answer')
    expect(read.result).toContain('partial review notes')
  })
  ...
  it('fails the job without ever starting the CLI when the probe refuses', async () => {
    ...
    const read = jobs.read(id)
    expect(read.job.status).toBe('failed')
    expect(read.result).toContain('the Command Code CLI is not installed')
