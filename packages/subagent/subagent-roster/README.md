...
|---|---|
| `resolvedRoster(workspace, signal)` | The roles enabled for one workspace — `name`, `purpose`, `whenToUse`, `invocation`, the resolved `model` policy, `access`, and `execution` — each carrying per-field override source, plus the canonical workspace key they resolved for. A role this workspace disabled or removed is absent |
