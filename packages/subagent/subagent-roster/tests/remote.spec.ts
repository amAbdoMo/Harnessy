 ...
 * with their per-field source, the stored document it edits, the routing
...
import { canonicalWorkspaceKey } from '../src/settings.ts'
import type {
  SubagentDefinition,
  SubagentRoleView,
  SubagentSettings,
...
/** Source for a definition no workspace layer names at all. */
const INHERITED = {
...
describe('roster Remote surface', () => {
  ...
  it('reports an override that names the global value as overridden', async () => {
    // Source records that the workspace layer decided this field, which is
