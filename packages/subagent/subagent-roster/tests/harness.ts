...
export async function bootRoster(options: RosterOptions = {}): Promise<Roster> {
  ...
  ctx.sessionProjections.register(roster.delegationAuthorityProjectionDefinition)
...
export interface PluginRoster {
  ...
  /** Replace the live Config snapshot exposed to the next operation. */
  configure(settings: SubagentSettings): void
 ...
 */
export async function bootPlugin(settings?: SubagentSettings): Promise<PluginRoster> {
  const ctx = new Context()
  let current = settings ?? defaultSubagentSettings()
  const config = { get: () => current } as Volatile<SubagentSettings>
  await mountAgentLoopTestDependencies(ctx)
  ctx.sessionProjections.register(turnBoundaryProjectionDefinition)
  await ctx.plugin(SubagentRuntime)
  ...
  const fiber = await ctx.plugin({
    name: 'roster-test-plugin',
    inject: roster.inject,
    apply(pluginCtx: Context): void { roster.apply(pluginCtx, config) },
  })
