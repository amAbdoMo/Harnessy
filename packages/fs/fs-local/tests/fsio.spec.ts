...
describe('resolveLocalTarget', () => {
  ...
  it.skipIf(!symlinksUsable())('two paths to the same file via a symlink share one targetKey', async () => {
  ...
  // A real symbolic link needs Developer Mode or SeCreateSymbolicLinkPrivilege on Windows.
  it.skipIf(!symlinksUsable())('keeps the key stable across create when an ancestor is a symlink', async () => {
...
describe('probeNoFollow', () => {
  // A real symbolic link needs Developer Mode or SeCreateSymbolicLinkPrivilege on Windows.
  it.skipIf(!symlinksUsable())('reports symlinks without following them', async () => {
    const real = join(dir, 'real.txt')
    const link = join(dir, 'link.txt')
    await writeFile(real, 'hi')
    await symlink(real, link)
...
describe('listDirectory', () => {
  // A real symbolic link needs Developer Mode or SeCreateSymbolicLinkPrivilege on Windows.
  it.skipIf(!symlinksUsable())('lists direct children in stable order without reading content', async () => {
  ...
  // A real symbolic link needs Developer Mode or SeCreateSymbolicLinkPrivilege on Windows.
  it.skipIf(!symlinksUsable())('derives child target keys from the listed parent identity', async () => {
  ...
  // A real symbolic link needs Developer Mode or SeCreateSymbolicLinkPrivilege on Windows.
  it.skipIf(!symlinksUsable())('translates preflight metadata IO failures into FS_IO_ERROR', async () => {
  ...
  // A real symbolic link needs Developer Mode or SeCreateSymbolicLinkPrivilege on Windows.
  it.skipIf(!symlinksUsable())('translates child resolution failures into structured listing errors', async () => {
  ...
  // A real symbolic link needs Developer Mode or SeCreateSymbolicLinkPrivilege on Windows.
  it.skipIf(!symlinksUsable())('translates child permission failures into FS_PERMISSION_DENIED', async () => {
    const root = join(dir, 'listed')
