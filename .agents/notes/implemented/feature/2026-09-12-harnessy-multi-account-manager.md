...
Harnessy replaces the shell's default sidebar Settings launcher through the optional `settings.launcher` slot. The replacement shows the active Codex identity, uppercase plan, and animated `5h` and `7d` usage meters, then opens a compact menu with the same account summary and a Settings action. The launcher refreshes usage at application startup. The account action requests the existing Models-footer manager, so one account implementation owns refresh, switching, and exclusive-modal behavior. The settings shell continues to own panel visibility and direct section navigation.
