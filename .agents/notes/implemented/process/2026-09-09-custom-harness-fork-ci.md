...
[Custom Harness Windows](../../../../.github/workflows/custom-harness-windows.yml) is the automatic push and pull-request workflow for this fork. It runs on a hosted Windows x64 runner without product credentials, type-checks the desktop host, and exercises the focused packaging-policy tests that distinguish the personal unsigned installer from the signed upstream release path. A manual run, or a `master` push whose head commit contains `[build-installer]`, also builds the unsigned Windows x64 installer, executes the packaged-runtime verification, and uploads the resulting files as a workflow artifact. When the packaged runtime reports LibreOffice's exact `loadComponentFromURL returned an empty reference` native-load transient, the workflow preserves that failure and retries the smoke once in a fresh process against the completed package; another failure, or any other error, stops the job. A failed packaging run reports its newest redacted packaging journal in the check annotation and preserves the complete journal with the installer log in a diagnostic artifact. The successful installer artifact remains distinct from the signed upstream release path and is not uploaded to an update feed.
