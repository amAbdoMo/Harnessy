...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-workspace-brief': { kind: 'none', reason: 'Browser-side human-command action and persisted command-card renderer; registers nothing model-facing.' },
  ...
  'packages/workspace/workspace-brief': { kind: 'none', reason: 'Human-only read command whose log-only lifecycle and output are excluded from model history.' },
  ...
  'packages/bundle/custom-harness': { kind: 'indirect', reason: 'The bundle is a product patch carrier; inherited packages own every model-facing behavior.' },
  ...
  'packages/llm/model-capabilities': { kind: 'indirect', reason: 'The layer answers a discovery-time question the LLM service asks; only the levels a deployment then declares reach a request.' },
  ...
  'packages/test-support/platform-probe': { kind: 'none', reason: 'The probe reads one host capability for tests and registers nothing model-facing.' },
