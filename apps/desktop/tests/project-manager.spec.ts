...
describe('desktop project transactions', () => {
  ...
  it('reinstalls changed packaged contents even when the release version is unchanged', async () => {
    const root = temporaryRoot()
    const paths = resolveDesktopPaths(join(root, '.dsh'))
    const manager = new DesktopProjectManager(paths, { node: process.execPath, pnpm: writeFakePnpm(root) })
    const firstSeed = join(root, 'seed-1')
    createTestSeedMetadata(firstSeed, release())
    writeFileSync(join(firstSeed, 'pnpm-lock.yaml'), 'lockfileVersion: 9\n# first build\n')
    archiveStore(firstSeed)
    writeIntegrity(firstSeed)
    const firstIntegrity = readFileSync(join(firstSeed, 'integrity.json'), 'utf8')

    await expect(manager.applyRelease(firstSeed, '1.0.0', hooks())).resolves.toBe(true)
    await expect(manager.applyRelease(firstSeed, '1.0.0', hooks())).resolves.toBe(false)
    await manager.mutate({ type: 'plugin-add', spec: '@scope/plugin@2.0.0' }, hooks())
    const legacyWorkspace = readFileSync(join(paths.profile, 'pnpm-workspace.yaml'), 'utf8')
    writeFileSync(join(paths.profile, 'pnpm-workspace.yaml'), `# legacy core mapping\n${legacyWorkspace}`)

    const nextSeed = join(root, 'seed-2')
    createTestSeedMetadata(nextSeed, release())
    writeFileSync(join(nextSeed, 'pnpm-lock.yaml'), 'lockfileVersion: 9\n# corrected build\n')
    archiveStore(nextSeed)
    writeIntegrity(nextSeed)

    await expect(manager.applyRelease(nextSeed, '1.0.0', hooks())).resolves.toBe(true)
    expect(readFileSync(join(paths.profile, 'integrity.json'), 'utf8'))
      .toBe(readFileSync(join(nextSeed, 'integrity.json'), 'utf8'))
    expect(readFileSync(join(paths.profile, 'integrity.json'), 'utf8')).not.toBe(firstIntegrity)
    expect(manager.listPlugins()).toEqual([{ name: '@scope/plugin', version: '2.0.0' }])
    await expect(manager.applyRelease(nextSeed, '1.0.0', hooks())).resolves.toBe(false)
  })
})
