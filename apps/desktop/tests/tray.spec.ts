...
it('shows the application icon with its name as the tooltip and an Open / Quit menu', () => {
  ...
  expect(f.native.setToolTip).toHaveBeenCalledWith('Harnessy')
  expect(labels(native.menus[0]!)).toEqual(['Open Harnessy', 'separator', 'Quit Harnessy'])
...
it('relabels the menu in the current locale and ignores relabel after disposal', () => {
  ...
  f.setLocale('zh')
  f.tray.relabel()
  expect(labels(native.menus[1]!)).toEqual(['打开 Harnessy', 'separator', '退出 Harnessy'])
  f.tray.dispose()
  f.tray.dispose()
