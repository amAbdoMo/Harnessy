...
On Windows, the primary Harnessy window opens maximized with one 36px integrated title bar. The bar keeps native minimize, maximize, and close controls; its app-owned area provides a sidebar toggle plus native File, Edit, View, and Help menus. The main window remains resizable, while plugin management continues to open in a separate task-sized window with standard native chrome.

After the Windows tray icon is ready, closing the primary window hides it without stopping the Desktop Host or its active Sessions. Selecting the tray icon or **Open Harnessy** restores the same window; **Exit Harnessy** performs the explicit graceful shutdown. If tray creation fails, background mode stays disabled and closing the primary window exits, so the application cannot remain running without a reopen path.

The main dsh renderer receives the desktop protocol marker, one validated operation that opens a fixed native menu group, and one bounded operation that shows secret-free native notifications. A packaged Windows start repairs the Start-menu shortcut that binds the executable to Harnessy's application id, and the main process retains each notification until Windows closes, fails, or activates it. Notification clicks restore and focus the main window. The separate plugin window receives structured list, install, remove, update, and update-check operations; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.

Electron always uses its typed English shell copy. Menus, native dialogs, and the plugin-management renderer use the same locale payload; packaging retains only Chromium's `en-US` locale pack, and the repository Client UI i18n gate checks these desktop sources.

### Seed installation

The packaged seed is an installation kit, not a ready-to-run `node_modules` tree. Packaging creates the lockfile, materializes the production graph online with lifecycle scripts disabled, deletes `node_modules` and every temporary pnpm cache, config, and state directory, and proves one complete installation offline from the final store alone with the private Desktop Host entry and overlay present. A macOS build stages every Mach-O object from pnpm's content-addressed store, Developer ID signs at most four independent copies concurrently, and updates the affected SHA-512 index records only after all signers succeed. Another offline install proves the rewritten store before sharding; preparation then extracts the final archives and verifies every embedded signature. The signed seed retains the release identity, local first-party tarballs and their descriptor, repository-owned dependency patches and their descriptor, project metadata, lockfile, integrity inventory, and pnpm store content required to repeat that installation on the user's machine.
...
| `store-archives.json` and `store-archives/*.tar` | Validate the deterministic uncompressed shards, stream package files through four bounded I/O lanes into `$DSH_HOME/desktop/pnpm/store`, and transactionally merge staged pnpm SQLite indexes without removing packages already downloaded for Desktop plugins. |
| Project metadata, declared dependency patches, and `desktop-packages/` | Copy into a unique `$DSH_HOME/desktop/staging/<transaction-id>/profile` project and preserve its patch mappings during plugin changes. |
...
3. Otherwise validate every archive entry while extracting only the small SQLite indexes into a transaction directory. Four bounded workers perform both archive passes so the disk remains responsive while independent shards progress concurrently. The second pass streams package files directly into the private store, then merges the staged indexes before creating a staging profile and running `pnpm install --offline --frozen-lockfile --trust-lockfile` through the bundled Node.js and pnpm. Seed records replace matching index keys while plugin-only records remain available; no complete temporary store is copied a second time.
...
An unpacked artifact contains four independent size contributors: Electron, the offline seed store shards and local dsh tarballs, the upstream Node.js and pnpm runtime, and the small shell application. Electron carries only the `en-US` Chromium locale pack; GPU fallbacks, licenses, the upstream Node.js executable, and the offline seed remain complete. The shards are uncompressed so the outer DMG, ZIP, or NSIS compressor and differential updater can operate on stable ranges. Filesystem size is not installer download size, so measure both separately. First packaged startup also streams the seed store into `$DSH_HOME/desktop/pnpm/store` before installing the writable profile, so release qualification must measure both application and Harness-home disk use.
