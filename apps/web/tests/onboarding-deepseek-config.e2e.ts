...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('configures arbitrary DeepSeek models and prompts after the selected model is removed', async () => {
    ...
    // The row's own disclosure carries its capacities, as in the pi-ai form.
    await settings.getByRole('button', { name: '能力 3' }).click()
    ...
    await page.getByRole('option', { name: /DeepSeek-V4-Flash-Vision-Exp/ }).waitFor({ timeout: 10_000 })
    await page.getByRole('option', { name: /Private Preview/ }).waitFor({ timeout: 10_000 })
    expect(tripwire.warnings).toEqual([])
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
